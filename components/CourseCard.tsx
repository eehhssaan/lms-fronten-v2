...
export default function CourseCard({ course, showEnrollmentStatus = false }: CourseCardProps) {
  const { user } = useAuth();
  ...
  const isEnrolled = user && course.students ? 
        ...
        <Box mt="auto">
          <Flex mb={1} justifyContent="space-between" alignItems="center">
            <Text fontSize={1} color="secondary">
              <Text as="span" fontWeight="bold">Start:</Text> {startDate}
            </Text>
          ...
          <Flex mb={2} justifyContent="space-between" alignItems="center">
            <Text fontSize={1} color="secondary">
              <Text as="span" fontWeight="bold">End:</Text> {endDate}
            </Text>
            
            {course.teacher && (
              <Text fontSize={1} color="secondary">
                <Text as="span" fontWeight="bold">Instructor:</Text> {course.teacher.firstName} {course.teacher.lastName}
              </Text>
            )}
          </Flex>
          ...
          <Flex justifyContent="space-between" alignItems="center">
            ...
            <Text fontSize={1} color="secondary">
              {course.students ? `${course.students.length}/${course.maxStudents} students` : '0/0 students'}
            </Text>
          </Flex>
        </Box>
      </Card>
